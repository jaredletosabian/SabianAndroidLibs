...
object TooLargeTool {
    ...
    fun bundleBreakdown(bundle: Bundle): String {
        ...
        for ((key1, totalSize1) in subTrees) {
            ...
            if (key1 == "androidx.lifecycle.BundlableSavedStateRegistry.key") {
                result += "\n***%s".format(bundleBreakdown(bundle.getBundle(key1)!!))
            }

            if (key1 == "android:support:fragments") {
                val fragmentsBundle = bundle.getBundle("android:support:fragments")!!
                result += "\n*****frags %s".format(bundleBreakdown(fragmentsBundle))
            }
        }
        return result
    }
